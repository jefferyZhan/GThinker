...
class SYSTEMRLHFDataset(Dataset):
    """
    We assume the dataset contains a column that contains prompts and other information
    ZYF Modify: Support SYSTEM PROMPT INPUT
    """
    ...
    def _build_messages(self, example: dict[str, Any]) -> list[dict[str, Any]]:
        # ZYF Modify: To support SYSTEM PROMPT Training
        prompt_str: str = example[self.prompt_key]
        full_list = []
        if self.format_prompt:
        ...
        if self.image_key in example:
            # https://huggingface.co/docs/transformers/en/tasks/image_text_to_text
            content_list = []
            # Check image and <image> token match
            image_token_num = prompt_str.count("<image>")
            if image_token_num == len(example[self.image_key]):
                pass
            else:
                prompt_str = "<image> " * (len(example[self.image_key])-image_token_num) + prompt_str
            for i, content in enumerate(prompt_str.split("<image>")):
                if i != 0:
                    content_list.append({"type": "image"})
                ...
                if content:
                    content_list.append({"type": "text", "text": content})
            full_list.append({"role": "user", "content": content_list})
            return full_list
        elif self.video_key in example:
            content_list = []
            for i, content in enumerate(prompt_str.split("<video>")):
            ...
            full_list.append({"role": "user", "content": prompt_str})
            return full_list
    ...
    def __getitem__(self, index):
        ...
        # with open("/mnt/bn/ecomckptnas/yufeizhan/valleyomni_training_yamls/EasyR1/reward_input_debug.jsonl", "w") as f:
        #     f.writelines([json.dumps(messages, ensure_ascii=False, indent=4) + "\n"])
        if self.image_key in example:
            prompt = self.processor.apply_chat_template(messages, add_generation_prompt=True, tokenize=False)
            images = example.pop(self.image_key)
            if self.image_dir is not None and len(images) != 0 and isinstance(images[0], str):  # image paths
