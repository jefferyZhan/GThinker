...
SequentialDiffRewardFunction = Callable[[RewardInput], RewardScore]
...
class SequentialDiffFunctionRewardManagerMixin:
    reward_fn: SequentialDiffRewardFunction

    def compute_reward_sequential_diff(self, data: DataProto) -> Tuple[torch.Tensor, dict[str, list[float]]]:
        reward_tensor = torch.zeros_like(data.batch["responses"], dtype=torch.float32)
        reward_metrics = defaultdict(list)
        response_ids = data.batch["responses"]
            ...
            score = self.reward_fn(
                {
                    ...
                    "response_length": cur_response_length,
                    "ground_truth": data.non_tensor_batch["ground_truth"][i],
                    "type": data.non_tensor_batch["type"][i],
                }
            )
            reward_tensor[i, cur_response_length - 1] = score["overall"]
            for key, value in score.items():
...
class AutoRewardManager(BatchFunctionRewardManagerMixin, SequentialFunctionRewardManagerMixin):
    ...
    def compute_reward(self, data: DataProto) -> Tuple[torch.Tensor, dict[str, list[float]]]:
        ...
        elif self.reward_type == "sequential_diff":
            return self.compute_reward_sequential_diff(data)
        else:
