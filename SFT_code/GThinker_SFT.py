...
class SFTConfig(trl.SFTConfig):
    ...
    data_mode: Optional[str] = field(
        default="Direct",
        metadata={"help": "The mode of data formatting, support Direct, VR, Rationale"},
    )
...
def convert_example_VR_SYSTEM(example):
    """
    ...
    "<think> reasoning process here </think><answer> answer here </answer>. In the reasoning process enclosed within <think> </think>, each specific "
    "visual cue is enclosed within <vcues_*>...</vcues_*>, where * indicates the index of the specific cue. "
    "Before concluding the final answer, pause for a quick consistency check: verify whether the visual cues support the reasoning and whether each step logically follows from what is seen. "
    "If correct, conclude the answer; otherwise, revise the visual cues and reasoning, then conclude."
        )
        messages.append({
            "role": "system",
    ...
    if choices is not None:
        for i, choice in enumerate(choices): 
            choice_string += "{}. {}\n".format(chr(i+65), choice)
        pb = f"Question: {problem}\nOptions:\n{choice_string}\n{PROMPT}"
    ...
    answer_with_think = example.get("new_answer")
    messages.append({
        "role": "assistant",
        "content": answer_with_think,
    })
...
def convert_example_Direct(example):
    """
    correct example into "messages" 
    eg:
    {
...
def collate_fn(examples, flag="Direct"):
    
    if flag == "Direct":
        convert_example = partial(convert_example_Direct)
    elif flag == "VR_SYSTEM":
        convert_example = partial(convert_example_VR_SYSTEM)
    else:
...
def main(script_args, training_args, model_args):
    ...
    if training_args.data_mode == "Direct":
        convert_example = partial(convert_example_Direct)
    elif training_args.data_mode == "VR_SYSTEM":
        convert_example = partial(convert_example_VR_SYSTEM)
    #logger.info("Dataset Example:\n {} \n {}".format(convert_example(dataset[0])["messages"][0]["content"][0]["text"],convert_example(dataset[0])["messages"][1]["content"]))
    ...
    trainer = SFTTrainer(
        ...
        data_collator=partial(collate_fn, flag=training_args.data_mode),
